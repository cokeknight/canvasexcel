...
DataGrid.prototype.paintCellText = function (dc, cell, x, y, width, height) {
  ...
  var cellvalue = DataCell.getValue(cell),

    textFormat = DataCell.getTextFormat(cell,this._swty),

    textfont = this.getdcfont(DataCell.getFtid(cell)),

    textAlign = cell._hag || this._hag,

    textverAlign = DataCell.getTextverAlign(cell) || this._vag,

    textfontsize = DataCell.getFontSize(cell,ftlist);
  ...
  if ((cell._fl >> 8) & 0x01) { //fl 8 自动换行
    ...
    if (DataCell.getuline(cell,ftlist)) { //下划线
      dc.beginPath();
