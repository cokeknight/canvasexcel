...
DataGrid.prototype.getCellSize = function(rowindex, colindex) {
  ...
  for (i = 0; i < DataCell.getColspan(cell); i++) {
  ...
  for (i = 0; i < DataCell.getRowspan(cell); i++) {
...
DataGrid.prototype.findCombineCellsAroundSelCells = function() // 找到合并的单元格
{
  ...
  for (i = this._sel_startRow; i <= this._sel_endRow; i++) {
    ...
    if (DataCell.getRowspan(cell) <= 0 && DataCell.getColspan(cell) <= 0) {
      temp_startrow = 0 - DataCell.getRowspan(cell)
      temp_startcol = 0 - DataCell.getColspan(cell)
      cell = this._cells[temp_startrow][temp_startcol]
      temp_endrow = temp_startrow + DataCell.getRowspan(cell) - 1
      temp_endcol = temp_startcol + DataCell.getColspan(cell) - 1
    ...
    } else if (DataCell.getRowspan(cell) > 1 || DataCell.getColspan(cell) > 1) // mouseDown 触发合并单元格
    {
      temp_startrow = i
  ...
  for (i = this._sel_startRow; i <= this._sel_endRow; i++) {
    ...
    } else if (DataCell.getRowspan(cell) > 1 || DataCell.getColspan(cell) > 1) {
      temp_startrow = i
...
DataGrid.prototype.getScrollTop = function() {
  ...
  for (i = 0, tempheight = 0; i < this._scrollRowNum; i++) {
    row = (this._rows[i])
    if (DataRow.getVisible(row)) {
      tempheight += DataRow.getHeight(row)
    }
  }
  return tempheight
}
...
DataGrid.prototype.getScrollLeft = function() {
  ...
  for (i = 0, tempwidth = 0; i < this._scrollColNum; i++) {
    col = this._cols[i]
    if (DataCol.getVisible(col)) {
      tempwidth += DataCol.getWidth(col)
    }
  }
  return tempwidth
}
...
DataGrid.prototype.getFullWidth = function() {
  ...
  for (i = 0, fullwidth = 0; i < this._cols.length; i++) {
    col = this._cols[i]
    if (DataCol.getVisible(col)) {
      fullwidth += DataCol.getWidth(col)
    }
  }
  col = this._cols[this._cols.length - 1]
  return fullwidth + this.getGridZoneWidth() - DataCol.getWidth(col)
}
...
DataGrid.prototype.getFullHeight = function() {
  ...
  for (i = 0, fullheight = 0; i < this._rows.length; i++) {
    row = (this._rows[i])
    if (DataRow.getVisible(row)) {
      fullheight += DataRow.getHeight(row)
    }
  }
  ...
  for (i = 0; i < this._rows.length; i++) {
    ...
    if (DataRow.getVisible(row)) {
      if (DataRow.getColor(row) !== false) {
        ...
        // dc.fillRect(x,y,width,DataRow.getHeight(row));
      }
    }
...
DataGrid.prototype.paintCellContent = function(dc, cell, x, y, width, height) {
  const backFillStyle = DataCell.getBackColor(cell,this._brlist)
