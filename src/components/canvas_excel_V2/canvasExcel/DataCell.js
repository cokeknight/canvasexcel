...
// function DataCell(config,dc) {
//   this.copyConfig(config,dc)
//   /*
//      this._value
//      this._fontFamily
//      this._fontSize
...
// 	 */
// }
// DataCell.prototype.copyConfig = function(config,dc) {
//   if (config.t !== undefined) { // 单元文本内容。
//     this._t = config.t
//   }
//   if (config.value !== undefined) { // 单元文本内容。
//     this._t = config.value
...
//     this._fl = undefined
//   }
//   if (config.frozen != undefined) { // 单元格锁定
//     this.fl = this._fl=''
//     this.fl&= ~(1 << 0)
//     this._fl&= ~(1 << 0)
//   }
...
//   }
//   if (config.tableHeader !== undefined) { // 是表格的表头
...
//   }
//   if (/^\d+(\.\d+)?$/.test(this.value) &&(this._hag === '' || this._hag === undefined)) {
//     this._hag = '2'
//   }
//   if(this.value && dc) {
//     this.textlength = dc.measureText(this.value).width
//   }
...
//   return this.value || ''
// }
// DataCell.prototype.setValue = function(val) {
//   this.value = this._t = val
//   return this
// }
...
// 		return "none";
// 	}
// 	if(this.getRowspan() <= 0 && this.getColspan() <=0)
// 	{
// 		return "combined";
// 	}
...
// 		return false;
// 	}if(this._t!= "" && this._t!=undefined)
// 	{
// 		return true;
// 	}
// 	if(this._tpenid!=undefined)
// 	{
// 		return true;
// 	}
...
export default {
  getTextverAlign(config){
    if (config.vag) {
      return config.vag
    } 
    return '6'
  },
  getBorderLine(config) {
    return config.borderline
  },
 getuline(config,obj){
		if(config.ftid!==undefined){
			var ftid=parseInt(config.ftid,10);
			if(obj[ftid]!==undefined){
				if(obj[ftid].uline!==undefined){
					return 1;
				}
  ...
  getTextFormat (config,swty) {
    ...
    let weishu = 2;
    let format = ''
    if (config.dpt !== undefined) {
      weishu = 1
    }
    if (swty !== undefined) {
      format = setswty[swty]
    } else {
      format = setswty[config._swty]
    }
  ...
  getColspan(config){
    ...
    var cols=parseInt(config.cols,10);
    if(cols<0){
      //if(cols === -1)return -1;
      return -( - cols - 1);
    }if(cols>=0){
  ...
  getBackFillColor(config){
    if (config.backColor) {
      return config.backColor
    }
    return false
  },
  getBackColor(config,obj) {
    if (config.brid !== undefined) {
      return String(obj[config._brid])// T.color.getcolorFromByte(String(obj[this._brid].color));//cell._backColor;
  ...
  getCombineStyle(config)
  {
    if(this.getRowspan(config) == 1 && this.getColspan(config) == 1)
    {
      return "none";
    }
    if(this.getRowspan(config) <= 0 && this.getColspan(config) <=0)
    {
      return "combined";
    }
    return "combine";
  ...
  ifPaint (config){
  {
  	if(config._visible === false && config._note === 'none'){
  		return false;
  	}if(config._t!= "" && config._t!=undefined)
  	{
  		return true;
  	}
  	if(config._tpenid!=undefined)
  	{
  		return true;
  	}
  	else if(config._lpenid!=undefined)
  	{
  		return true;
  	}
  	else if(config._rpenid!=undefined)
  	{
  		return true;
  	}
  	else if(config._bpenid!=undefined)
  	{
  		return true;
  	}
  	//else if(config._backColor != "")
  	//{
  		//return true;
  	//}/
  	else if(config._brid != undefined)
  	{
  		return true;
  	}
  	else if(config.rows != 0 && config.rows != undefined)
  	{
  		return true;
  	}
  	else if(config.cols != 0 && config.cols != undefined)
  	{
  		return true;
  	}
  	else if(config._slantLine!='none')
  	{
  		return true;
  	}
  	else if(config._bcor!=undefined)
  	{
  		return true;
  	}
  	else if(config._cellImage!='none')
  	{
  		return true;
  	}else if(config._imgid != undefined)
  	{
  		return true;
  	}else if(config._controlsItem != 'none')
  	{
  		return true;
  	}else if((config._fl >> 12) & 0x01 || (config._fl >> 10) & 0x01
  	|| (config._fl >> 5) & 0x01 || (config._fl >> 15) & 0x01
  	 || (config._tag >> 0) & 0x01 || (config._tag >> 3) & 0x01 || (config._tag >> 4) & 0x01 || (config._tag >> 5) & 0x01
  	  || (config._fl >> 12) & 0x01)
  	{
  		return config;
  	}else if(config._slty != undefined){
  		return config;
  	}
  	else if(config._note != 'none')
  	{
  		return true;
  	}
  	else
