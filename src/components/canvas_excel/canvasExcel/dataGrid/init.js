  ...
  // } else {
    ...
    for (i = 0; i < this.rowsNum; i++) {
      if (config.rows && config.rows[i]) {
        this._rows.push(new DataRow(config.rows[i]))
      } else {
        this._rows.push(new DataRow(this._rows[i] || {}))
      }
    }
  // }
  ...
  // } else {
    ...
    for (i = 0; i < this.colsNum; i++) {
      if (config.cols && config.cols[i]) {
        this._cols.push(new DataCol(config.cols[i]))
      } else {
        this._cols.push(new DataCol(this._cols[i] || {}))
      }
    }
  // }
    ...
    for (i = 0; i < this._rows.length; i++) {
      ...
      for (j = 0; j < this._cols.length; j++) {
        if (config.cells && config.cells[i] && config.cells[i][j]) {
          this._cells[i].push(new DataCell(config.cells[i][j], dc))
        } else {
